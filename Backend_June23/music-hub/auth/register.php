...
if(isset($_POST['submit'])) {
  ...
  if($name!="" || $email!="" || $pwd!="" || $cpwd!="") {
    if($pwd==$cpwd) {
      $query = "SELECT count(*) as usercount FROM users WHERE email='$email';";
      $result = mysqli_query($conn,$query);
      $rows = mysqli_fetch_array($result);
      
      if($rows['usercount']==0) {
        $pwdhash = password_hash($pwd,PASSWORD_BCRYPT);
        $query = "INSERT INTO users (name,email,password) VALUES ('$name','$email','$pwdhash');";
        $result = mysqli_query($conn,$query);
        if($result==1) {
          header("Location:./login.php");
        }
        else {
          $errmsg = "Account could not be created. Try again.";
        }
      }
      else {
        $errmsg = "Email used has an active account. Try logging in";
